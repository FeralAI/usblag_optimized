    ...
    if (descr_type == USB_DESCRIPTOR_INTERFACE) {
      ...
      if (intf_ptr->bInterfaceClass == 0xFF && (
        ...
        *isXboxOne = intf_ptr->bInterfaceSubClass == 0x47;
        found = true;
        /*Serial.print("Is XboxOne? ");
        Serial.print(*isXboxOne);
        Serial.print("\n");*/
      }
    } else if (found && descr_type == USB_DESCRIPTOR_ENDPOINT) {
      USB_ENDPOINT_DESCRIPTOR* ep_ptr = ( USB_ENDPOINT_DESCRIPTOR* )buf_ptr;
      /*  Serial.print("End point found: ");
        Serial.print(ep_ptr->bEndpointAddress);
        Serial.print("\t");
        Serial.print(ep_ptr->bmAttributes);
        Serial.print("\n");*/
      if ((ep_ptr->bEndpointAddress & 0xF0) == 0x80) {
...
uint8_t XInputDriver::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        epInfo[ XBOX_INPUT_PIPE ].epAddr = isXboxOne ? 0x02 : 0x01; // XBOX 360 report endpoint
        ...
        epInfo[ XBOX_OUTPUT_PIPE ].epAddr = isXboxOne ? 0x02 : 0x02; // XBOX 360 output endpoint
